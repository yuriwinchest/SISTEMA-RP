...
if (empty($usuario_selecionado)) {
	...
	$sql_usuario = " usuario = '$usuario_selecionado' and ";
}
...
$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and data <= curDate() and hora < curTime() and empresa = '$id_empresa' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$tarefas_atrasadas = @count($res);

$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and empresa = '$id_empresa' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$tarefas_pendentes = @count($res);

$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and data = '$data_atual' and empresa = '$id_empresa' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$tarefas_hoje = @count($res);

$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Concluída' and data >= '$dataInicial' and data <= '$dataFinal' and empresa = '$id_empresa' order by id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$tarefas_concluidas = @count($res);


if ($filtro == 'Atrasadas') {
	$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and data <= curDate() and hora < curTime() and empresa = '$id_empresa' order by hora asc");
} else if ($filtro == 'Pendentes') {
	$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and empresa = '$id_empresa' order by hora asc");
} else if ($filtro == 'Hoje') {
	$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Agendada' and data = '$data_atual' and empresa = '$id_empresa' order by hora asc");
} else if ($filtro == 'Concluidas') {
	$query = $pdo->query("SELECT * from $tabela where $sql_usuario  status = 'Concluída' and empresa = '$id_empresa' order by hora asc");
} else {
	$query = $pdo->query("SELECT * from $tabela where $sql_usuario  data >= '$dataInicial' and data <= '$dataFinal' and empresa = '$id_empresa' order by  CASE 
...
HTML;
	...
	for ($i = 0; $i < $linhas; $i++) {
		...
		$query2 = $pdo->query("SELECT * FROM clientes where id = '$usuario'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if (@count($res2) > 0) {
...
<td align="center">{$nome_usuario}</td>
