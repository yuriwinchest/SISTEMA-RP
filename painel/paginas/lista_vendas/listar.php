...
$query = $pdo->query("SELECT * from $pagina where pago = 'Não' and referencia = 'Venda' and empresa = '$id_empresa'");
...
if($vencidas == 'Vencidas'){
	$query = $pdo->query("SELECT * from $pagina where vencimento < curDate() and pago = 'Não' and referencia = 'Venda' and empresa = '$id_empresa' order by id desc ");
}
	...
	$query = $pdo->query("SELECT * from $pagina where vencimento = '$data_amanha' and pago = 'Não' and referencia = 'Venda' and empresa = '$id_empresa' order by id desc ");
}
else if($vencidas == 'Canceladas'){
	$query = $pdo->query("SELECT * from $pagina where cancelada = 'Sim' and referencia = 'Venda' and empresa = '$id_empresa' order by id desc ");
}
else{
	$query = $pdo->query("SELECT * from $pagina WHERE vencimento >= '$dataInicial' and vencimento <= '$dataFinal' and pago LIKE '$status' and referencia = 'Venda' and empresa = '$id_empresa' order by id desc ");
}
...
$total_venda = $res[$i]['total_venda'];
$hora_alerta = $res[$i]['hora_alerta'];
...
$query2 = $pdo->query("SELECT * FROM receber WHERE referencia = 'Venda' and empresa = '$id_empresa' AND data_lanc = '$data_lanc' AND hora_alerta = '$hora_alerta' AND total_venda = '$total_venda' ");
$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
if(@count($res2) > 0){
...
echo <<<HTML
			<tr> 
				<td><i class="fa fa-square {$classe_pago} mr-1"></i> {$descricao}</td> 
					<td>R$ {$total_vendaF} <small><a href="#" onclick="mostrarResiduos('{$id}')" class="text-danger" title="Ver Resíduos">{$vlr_antigo_conta}</a></small></td>	
					<td>{$nome_pessoa}</td>
					<td>{$data_lancF}</td>
				<td>{$data_vencF}</td>				
