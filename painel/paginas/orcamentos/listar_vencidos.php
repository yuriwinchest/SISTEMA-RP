...
$query = $pdo->query("SELECT * FROM $pagina WHERE status = 'Pendente' AND empresa = '$id_empresa' AND DATE_ADD(data, INTERVAL dias_validade DAY) < CURDATE()");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_orcamentos = @count($res);


$query = $pdo->query("SELECT * from $pagina WHERE status = 'Pendente' and empresa = '$id_empresa' AND DATE_ADD(data, INTERVAL dias_validade DAY) < CURDATE() order by id asc ");
...
echo <<<HTML
	...
	<br>
	<div align="right"><span>Total Vencidos: <span class="text-danger">{$total_pendentesF}</span></span> </div>
</small>
HTML;
}else{
