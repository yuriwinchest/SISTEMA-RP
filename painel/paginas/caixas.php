	...
	<div class="left-content mt-2 mb-3">
		<form action="rel/caixas_class.php" method="post" target="_blank">
			...
			<div class="esc ocultar_mobile" style="float:left; margin-right:30px">
				<input type="date" class="form-control " name="dataFinal" id="data-final" value="<?php echo $data_final_mes ?>"
			...
			<select class="form-select sel50"  name="operador" id="status-busca" style="width:350px" onchange="buscar()">
				...
				if ($nivel_usuario == 'Administrador' || $nivel_usuario == 'Gerente') {
					$query = $pdo->query("SELECT * FROM usuarios where nivel != 'Cliente' and acessar_painel != 'Não' and empresa = '$id_empresa' order by id desc");
						...
						<div class="col-md-5">
							<div class="form-group">
								...
								<select class="form-select sel5" aria-label="Default select example" name="operador" id="operador" style="width:100%" required>
									<?php
									if ($nivel_usuario == 'Administrador' || $nivel_usuario == 'Gerente') {
										$query = $pdo->query("SELECT * FROM usuarios where nivel != 'Cliente' and empresa = '$id_empresa' order by id desc");
										echo '<option value="">Selecionar Operador</option>';
									} else {
										$query = $pdo->query("SELECT * FROM usuarios where id = '$id_usuario' and empresa = '$id_empresa' ");
									}
