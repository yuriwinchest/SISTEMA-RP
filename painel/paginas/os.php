...
<div class="justify-content-between">
	<form action="rel/lista_os_class.php" target="_blank" method="POST">
		<div class="left-content mt-2">
			...
			<a style="margin-bottom: 20px; margin-top: 20px" class="btn ripple btn-primary text-white" onclick="inserir()" type="button"><i class="fa fa-file-circle-plus me-1"></i>Adicionar <?php echo ucfirst($pag); ?></a>
...
<div class="row">
	...
	<div class="col-md-12" align="center">
		...
		<a style="width: 100px; background-color: green; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-success" title="Todas" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('').change(); ">TODAS</a>	
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-danger" title="Abertas" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Aberta').change(); ">ABERTAS</a>
		<a style="width: 100px; background-color: blue; border-color: blue; font-size:10px; margin-bottom: 3px; " href="#" title="Iniciadas" class="btn btn-primary" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Iniciada').change(); ">INICIADAS</a>
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-warning" title="Aguardando" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Aguardando_Peca').change(); ">AGUARDANDO</a>
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; background-color: #c15b0a" href="#" class="btn btn-warning" title="Aguardando Aprovação" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Aguardando Aprovação').change(); ">APROVAÇÃO</a>
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-success" title="Finalizadas" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Finalizada').change(); ">FINALIZADAS</a>
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-info" title="Entregues" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Entregue').change(); ">ENTREGUES</a>
		<a style="width: 100px; font-size:11px; margin-bottom: 3px; " href="#" class="btn btn-secondary" title="Sem Reparo" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Sem_Reparo').change(); ">S/ REPARO</a>
		<a style="width: 110px; font-size:11px; margin-bottom: 3px" href="#" title="Não Aprovadas" class="btn btn-dark" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Nao_Aprovada').change(); ">N/ APROVADAS</a>
		<a style="width: 120px; font-size:11px; margin-bottom: 3px;  background-color: #d12f0f; color: white;" href="#" title="Não Aprovadas" class="btn" onclick="$('#dataFiltro').val('Tudo'); $('#status').val('Hoje').change(); ">ENTREGUES HOJE</a>
