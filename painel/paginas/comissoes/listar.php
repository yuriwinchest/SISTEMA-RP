...
if($funcionario == ""){
	...
	$sql_func = " and funcionario = '$funcionario'";
}
...
$query = $pdo->query("SELECT * FROM $tabela where empresa = '$id_empresa' and data_lanc >= '$dataInicial' and data_lanc <= '$dataFinal' and pago LIKE '$status' $sql_func and referencia = 'Comissão' ORDER BY pago asc, vencimento asc");
...
<td>
	...
	<big><a href="#" class="btn btn-danger-light btn-sm" onclick="excluir('{$id}')" title="Excluir"><i class="fa fa-trash-can"></i></a></big>
		   ...
		   <div class="dropdown head-dpdn2" style="display: inline-block;">                      
            <a class="btn btn-success-light btn-sm {$visivel}" title="Baixar Conta" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown {$visivel}"><big><i class="fa fa-check-square text-verde "></i></big></a>
