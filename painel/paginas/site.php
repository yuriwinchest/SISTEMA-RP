...
$query = $pdo->query("SELECT * from recursos_site where empresa = '$id_empresa' order by id desc limit 1");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$posicao = @$res[0]['posicao_recurso'];
$nova_posicao = $posicao + 1;	

$query = $pdo->query("SELECT * from perguntas_site where empresa = '$id_empresa' order by id desc limit 1");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    <nav>
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            ...
            role="tab" aria-controls="nav-home" aria-selected="true">
                <i class="fa fa-globe"></i> Topo Site
            </button>
            ...
            <button class="nav-link" id="nav-perguntas-tab" data-bs-toggle="tab" data-bs-target="#nav-perguntas" type="button"
            role="tab" aria-controls="nav-perguntas" aria-selected="false">
                <i class="fa fa-question"></i> Perguntas
            </button>

            <button class="nav-link" id="nav-rodape-tab" data-bs-toggle="tab" data-bs-target="#nav-rodape" type="button"
            role="tab" aria-controls="nav-rodape" aria-selected="false">
                <i class="fa fa-globe"></i> Rodapé
            </button>


            <button class="nav-link" id="nav-sobre-tab" data-bs-toggle="tab"
data-bs-target="#nav-sobre" type="button" role="tab" aria-controls="nav-sobre" aria-selected="false">
...
Sobre</button>
    ...
    <!-- Link do site alinhado à direita -->
    <div>
        <a href="<?php echo $url_sistema ?>site/<?php echo $url_site ?>" target="_blank" class="btn btn-primary">
            <i class="fa fa-external-link"></i> Acessar Site
        </a>
    </div>
</div>
...
<form id="form">
	<div class="tab-content" id="nav-tabContent">
		<div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
			...
			<div class="row">
				<div class="col-md-4 mb-2">
					<label>Fundo Topo (Retangular) Ex:1920 x 500  <a title="Excluir Imagem" href="#" onclick="excluirFundo('<?php echo $id_empresa ?>')"><i class="fa fa-trash-can text-danger"></i></a></label>
				...
				<div class="col-md-5 mb-2">
					<label>Fundo Topo (Vertical) Ex:500 x 900  <a title="Excluir Imagem" href="#" onclick="excluirFundoMobile('<?php echo $id_empresa ?>')"><i class="fa fa-trash-can text-danger"></i></a></label>
		...
		<div class="tab-pane fade" id="nav-sobre" role="sobre" aria-labelledby="nav-sobre-tab">
			...
			<div class="row">
			  <!-- Editor de texto (8 colunas) -->
			  <div class="col-md-8">
			    <label for="sobre_site">Descrição da Página Sobre</label>
			    <textarea id="sobre_site" name="descricao_site" class="form-control"><?php echo $descricao_site ?></textarea>
			  </div>

			  <!-- Campo de vídeo (4 colunas) -->
			  <div class="col-md-4">
			    <label for="video_url">URL do Vídeo Incorporado</label>
			    <input value="<?php echo $video_site ?>" type="text" id="video_url" name="video_site" class="form-control" placeholder="https://www.youtube.com/watch?v=... " oninput="mostrarVideo()">

			    <div class="mt-3">
			      <label>Pré-visualização:</label>
			      <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;">
			        <iframe id="video_preview" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allowfullscreen></iframe>
			      </div>
			    </div>
			  </div>
			</div>
...
<script>
  ...
  function mostrarVideo() {
    ...
    var iframe = document.getElementById('video_preview');
