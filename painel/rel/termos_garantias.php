...
$query = $pdo->query("SELECT * from termos_garantias where id = '$id'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if (@count($res) > 0) {
	$texto = $res[0]['texto'];
} else {
	$texto = 'Nenhum dado encontado!';
}


if (@$nome_sistema != "") {
	$texto = str_replace('{{EMPRESA}}', @$nome_sistema, @$texto);
}

if (@$cnpj_sistema != "") {
	$texto = str_replace('{{CNPJ}}', @$cnpj_sistema, @$texto);
}

if (@$endereco_sistema != "") {
	$texto = str_replace('{{ENDERECO}}', @$endereco_sistema, @$texto);
}

if (@$cidade_sistema != "") {
	$texto = str_replace('{{LOCAL}}', @$cidade_sistema, @$texto);
}

if (@$data_hoje != "") {
	$texto = str_replace('{{DATA}}', @$data_hoje, @$texto);
}
			...
			<table style="width: 100%; border: 0px solid #ccc;">
				<tr>
					<td style="border: 1px; solid #000; width: 30%; text-align: left;">
						<img style="margin-top: 0px; margin-left: 7px;" id="imag" src="<?php echo $url_sistema ?>img/logo.jpg"
							width="120px">
					</td>
