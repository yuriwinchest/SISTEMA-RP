		...
		<table id="cabecalhotabela" style="border-bottom-style: solid; font-size: 8px; margin-bottom:10px; width: 100%; table-layout: fixed;">
			<thead>
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC">
					...
					<td style="width:10%">TOTAL CUSTO</td>
					<td style="width:10%">VALOR LÍQUIDO</td>		
...
if($mostrar_registros == 'Não'){
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' and usuario_lanc = '$id_usuario' and empresa = '$id_empresa' order by id desc");
	}else{
	...
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' and empresa = '$id_empresa' order by id desc");
	}else{
...
$total_valor_contas += $valor;
$total_custo += $valor_custo;
$total_liquido = $total_valor_contas - $total_custo;
			...
			<thead>
				<tbody>
					<tr>
						...
						<td style="font-size: 10px; width:20px; text-align: right;"></td>
