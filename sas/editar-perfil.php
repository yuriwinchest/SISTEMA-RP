...
//validacao email
$query = $pdo->query("SELECT * from $tabela where email = '$email' and (empresa is null or empresa = 0)");
...
//validacao telefone
$query = $pdo->query("SELECT * from $tabela where telefone = '$telefone' and (empresa is null or empresa = 0)");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$id_reg = @$res[0]['id'];
if (@count($res) > 0 and $id != $id_reg) {
