...
if($mostrar_registros == 'Não'){
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber_sas where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' and usuario_lanc = '$id_usuario' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from receber_sas where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and usuario_lanc = '$id_usuario' order by id desc");
	}
}else{
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber_sas where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from receber_sas where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' order by id desc");
	}
}
