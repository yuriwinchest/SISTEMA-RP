...
if($mostrar_registros == 'Não'){
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from pagar_sas where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' and usuario_lanc = '$id_usuario' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from pagar_sas where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and usuario_lanc = '$id_usuario' order by id desc");
	}
}else{
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from pagar_sas where $tipo_data >= '$dataInicial' and $tipo_data < curDate() and pago != 'Sim' order by id desc");
	}else{
	$query = $pdo->query("SELECT * from pagar_sas where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' order by id desc");
	}
}
...
//PEGAR RESIDUOS DA CONTA
	...
	$query2 = $pdo->query("SELECT * FROM pagar_sas WHERE id_ref = '$id' and residuo = 'Sim'");
	$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
	if(@count($res2) > 0){
