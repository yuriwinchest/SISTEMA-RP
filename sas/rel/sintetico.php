...
if($filtro_tipo == "receber"){
	$filtro_tipo = "receber_sas";
}else{
	$filtro_tipo = "pagar_sas";
}
...
if($filtro_tabela == 'fornecedores'){
	if($mostrar_registros == 'Não'){
		$query9 = $pdo->query("SELECT * from $filtro_tabela where usuario = '$id_usuario' and (empresa = 0 or empresa is null) order by nome asc");
	}else{
		$query9 = $pdo->query("SELECT * from $filtro_tabela where (empresa = 0 or empresa is null) order by nome asc");
	}	
}else{
	$query9 = $pdo->query("SELECT * from $filtro_tabela where (empresa = 0 or empresa is null) order by nome asc");
}
