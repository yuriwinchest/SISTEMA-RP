					...
					<div class="row">
						<div class="col-md-6">
							...
							<select name="funcionario" id="funcionario" class="sel21" style="width:100%; height:35px">
								...
								<?php
								$query = $pdo->query("SELECT * from usuarios where nivel != 'Administrador' and (empresa = 0 or empresa is null) order by id asc");
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
								$linhas = @count($res);
								if ($linhas > 0) {
									for ($i = 0; $i < $linhas; $i++) {
						...
						<div class="col-md-6 mb-2">
							...
							<select name="fornecedor" id="fornecedor" class="sel20" style="width:100%; height:35px">
								...
								if ($mostrar_registros == 'Não') {
									$query = $pdo->query("SELECT * from fornecedores where usuario = '$id_usuario' and empresa = 0 order by id asc");
								} else {
									$query = $pdo->query("SELECT * from fornecedores where empresa = 0 order by id asc");
								}
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
								$linhas = @count($res);
								if ($linhas > 0) {
						...
						<div class="col-md-7">
							<div class="form-group">
								...
								<select class="form-select" name="frequencia" id="frequencia-parcelar" required style="width:100%;">
									<?php
									$query = $pdo->query("SELECT * FROM frequencias where empresa = 0 or empresa is null order by id desc");
									$res = $query->fetchAll(PDO::FETCH_ASSOC);
